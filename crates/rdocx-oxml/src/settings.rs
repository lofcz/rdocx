...
pub struct CT_Settings {
    ...
    update_fields_occurrences: usize,
...
impl CT_Settings {
    ...
    pub fn from_xml(xml: &[u8]) -> Result<Self> {
        ...
        Ok(Self {
            ...
            update_fields,
            update_fields_occurrences: update_fields_count,
            math_properties,
    ...
    pub fn set_update_fields(&mut self, value: Option<bool>) -> Result<()> {
        if self.update_fields_occurrences > 1
            || (self.update_fields_occurrences == 1 && self.update_fields.is_none())
        {
            return Err(OxmlError::InvalidValue(
                "cannot rewrite ambiguous or malformed w:updateFields".to_owned(),
            ));
        }
        if value.is_none() && self.update_fields_occurrences == 0 {
            return Ok(());
        }
        ...
        self.update_fields_occurrences = usize::from(value.is_some());
        Ok(())
    }
