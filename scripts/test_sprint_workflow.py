...
class SprintWorkflowTests(unittest.TestCase):
    ...
    def assert_rust_release_assets_contract(self, publish: str) -> None:
        ...
        self.assertIn("def normalize_text(payload: bytes) -> bytes:", aggregate)
        self.assertIn('return payload.replace(b"\\r\\n", b"\\n")', aggregate)
        self.assertIn(
    ...
    def assert_v0_12_1_release_notes_truth_contract(self, changelog: str) -> str:
    ...
    def assert_v0_13_1_release_notes_truth_contract(self, changelog: str) -> str:
        notes = workflow.render_release_notes(changelog, "v0.13.1")
        normalized_notes = " ".join(notes.split())
        for claim in (
            "M22 Word-depth boundary",
            "immutable v0.13.0 attempt",
            "shared OOXML 0.11.0 family",
            "No external issue or pull request belongs to the selected stable-family",
        ):
            self.assertIn(claim, normalized_notes, claim)
        for package in (
            ...
            self.assertIn(f"`{package}`", notes, package)
        self.assertIn("`rdocx-wasm@0.13.1` is not a crates.io package", normalized_notes)
        self.assertIn("Atul Sharma maintained the release", normalized_notes)
        self.assertNotIn("github.com/tensorbee/rdocx/issues/", notes)
        self.assertNotIn("github.com/tensorbee/rdocx/pull/", notes)
    ...
    def test_stable_release_family_is_prepared_at_0_14_0(self) -> None:
        ...
        self.assertIn("shared OOXML 0.12.1 family", " ".join(notes.split()))

    def test_release_notes_v0_13_1_cover_recovery_and_word_outcomes(self) -> None:
        changelog = (workflow.REPO / "CHANGELOG.md").read_text(encoding="utf-8")
        self.assert_v0_13_1_release_notes_truth_contract(changelog)

    def test_immutable_release_notes_v0_13_0_remain_recorded(self) -> None:
        changelog = (workflow.REPO / "CHANGELOG.md").read_text(encoding="utf-8")
    ...
    def test_s73_release_contract_requires_four_version_aligned_families(
        ...
        changelog = (workflow.REPO / "CHANGELOG.md").read_text(encoding="utf-8")
        tags = ("v0.14.0", "rpptx-v0.12.1", "py-rdocx-v0.14.0", "py-rpptx-v0.12.1")
        ...
        for tag in ("v0.14.0", "rpptx-v0.12.1"):
        ...
        for tag in ("py-rdocx-v0.14.0", "py-rpptx-v0.12.1"):
            notes = " ".join(workflow.render_release_notes(changelog, tag).split())
        ...
        self.assertIn("through F-X122", " ".join(plan.split()))
        notified = set()
    ...
    @unittest.skipUnless(
        ...
        "requires the separately published shared 0.12.1 family",
    )
    def test_prepared_rdocx_0_14_0_requires_published_shared_0_12_1(self) -> None:
        with tempfile.TemporaryDirectory(prefix="rdocx-registry-proof-") as temp:
            root = Path(temp)
            target = root / "package-target"

            def package_source(name: str, patches: dict[str, Path]) -> Path:
                command = [
                    "cargo",
                    "package",
                    "--locked",
                ...
                for dependency, path in patches.items():
                    command.extend(
                        (
                            "--config",
                            f'patch.crates-io.{dependency}.path="{path}"',
                        )
                    )
            ...
            rdocx_oxml = package_source("rdocx-oxml", {})
            source_patches = {"rdocx-oxml": workflow.REPO / "crates/rdocx-oxml"}
            rdocx_html = package_source("rdocx-html", source_patches)
            rdocx_layout = package_source("rdocx-layout", source_patches)
            rdocx_source = package_source(
                "rdocx",
                {
                    "rdocx-oxml": workflow.REPO / "crates/rdocx-oxml",
                    "rdocx-html": workflow.REPO / "crates/rdocx-html",
                    "rdocx-layout": workflow.REPO / "crates/rdocx-layout",
                },
            )

            normalized = tomllib.loads(
                (rdocx_source / "Cargo.toml").read_text(encoding="utf-8")
            )
            shared = (
                "oxml-chart",
                "oxml-core",
            ...
            for dependency in shared:
                ...
                self.assertEqual(requirement["version"], "0.12.1", dependency)
            ...
            (consumer / "Cargo.toml").write_text(
                "\n".join(
                    (
                        "[package]",
                        'name = "rdocx-registry-proof"',
                        'version = "0.0.0"',
                        'edition = "2024"',
                        "",
                        "[dependencies]",
                        f'rdocx = {{ path = "{rdocx_source}", default-features = false }}',
                        "",
                        "[patch.crates-io]",
                        f'rdocx-oxml = {{ path = "{rdocx_oxml}" }}',
                        f'rdocx-html = {{ path = "{rdocx_html}" }}',
                        f'rdocx-layout = {{ path = "{rdocx_layout}" }}',
                        "",
                    )
                ),
                encoding="utf-8",
            ...
            (consumer / "src/lib.rs").write_text(
                "pub fn package_class() -> rdocx::WordPackageClass {\n"
                "    rdocx::Document::new().package_class().unwrap()\n"
                "}\n",
                encoding="utf-8",
            )
            env = os.environ.copy()
            env["CARGO_HOME"] = str(root / "cargo-home")
            env["CARGO_TARGET_DIR"] = str(root / "consumer-target")
            ...
            for dependency in shared:
                ...
                for package in matches:
                    self.assertEqual(package["version"], "0.12.1", dependency)
                    self.assertTrue(
...
""",
    ...
    def test_incubating_release_family_is_prepared_at_0_12_1(self) -> None:
        ...
        self.assertEqual(
            ci.count(
                ...
                '"@tensorbee/rpptx-wasm" "0.12.1" "rpptx_wasm"'
            ),
            1,
        )
    ...
    def assert_release_preparation_metadata_contract(
        ...
        self.assertEqual(wasm_package["version"], "0.12.1")
        ...
        self.assertEqual(wasm_lock_versions, ("0.12.1",))
...
"""
    ...
    def test_rust_release_assets_are_complete_family_scoped_and_installable(
        ...
        mutations = {
            ...
            "unreviewed-license": publish.replace(
                'normalize_text(payloads["LICENSE"]) == normalize_text(expected_license)',
                'payloads["LICENSE"]',
                1,
            ),
    ...
    def test_rpptx_0_12_1_recovery_is_version_aligned_and_line_ending_safe(
        ...
        normalized = lambda payload: payload.replace(b"\r\n", b"\n")
        ...
        for member in (
            ...
            manifest = tomllib.loads(
                (workflow.REPO / f"crates/{member}/Cargo.toml").read_text(
                    encoding="utf-8"
                )
            )
        ...
        changelog = (workflow.REPO / "CHANGELOG.md").read_text(encoding="utf-8")
        for tag in ("rpptx-v0.12.1", "py-rpptx-v0.12.1"):
            self.assertIsNotNone(workflow.RELEASE_TAG_RE.fullmatch(tag), tag)
            self.assertIn(f"## {tag}\n", changelog)
